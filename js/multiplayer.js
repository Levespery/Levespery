...
function startGame(initialState, isOnline) {
  ...
  if (wallSelector) wallSelector.style.display = 'flex';
  ...
  if (isOnline) {
    ...
    // 隐藏重新开始按钮（在线模式下由房主控制）
    const controls = document.querySelector('#game-container .controls');
...
function endDrag(e) {
  ...
  // 强制清理所有 ghost 元素
  const ghosts = document.querySelectorAll('.wall-ghost');
